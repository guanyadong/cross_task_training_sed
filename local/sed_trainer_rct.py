...
from local.data_augm.rand_augm_agg import RandAugment
...
from .cross_task import (
    con_loss,
    cross_task_consistency,
    sod_label_generation,
    func_1,
    sod_2_label,
    cross_task_post_processing
)
...
class SEDTask4_2021(pl.LightningModule):
    ...
    def __init__(
        ...
        self.sod_student_05_buffer = pd.DataFrame()
    ...
    def training_step(self, batch, batch_indx):
        ...
        non_strong_mask = weak_mask + unsup_mask

        labels_weak = (torch.sum(labels[weak_mask], -1) > 0).float()

        # sod label generation
        labels_strong_sod = sod_label_generation(labels[strong_mask]).long()

        # sed student forward
        strong_preds_student, weak_preds_student, sod_student = self.detect(
        ...
        # sod supervised loss on strong labels
        sod_preds_strong = sod_student[strong_mask].permute(0, 2, 1)
        loss_strong_sod = self.cross_entropy_loss(
            sod_preds_strong, labels_strong_sod
        )

        # total supervised loss
        tot_loss_supervised = loss_strong + loss_weak + loss_strong_sod * 0.2

        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher, sod_teacher = self.detect(
                features, self.sed_teacher
            )

            sed2sod_teacher_new = func_1(strong_preds_teacher)

            sod_teacher_new = sod_2_label(sod_teacher)

            sod_mask = sod_teacher_new * sed2sod_teacher_new
            sod_mask_1 = sod_mask.sum(axis=-1) + 0     # cross-task consistency mask
            sod_mask_0 = 1 - sod_mask_1                # unconsist mask
            sod_mask_1 = sod_mask_1.unsqueeze(dim=-1)
            sod_mask_0 = sod_mask_0.unsqueeze(dim=-1)
            sod_mask_w_u_1 = sod_mask_1[non_strong_mask].detach()
            sod_mask_w_u_0 = sod_mask_0[non_strong_mask].detach()

        strong_self_sup_loss = con_loss(
            strong_preds_student, strong_preds_teacher,
        ...
        strong_sod_consis_loss = self.selfsup_loss(
            sod_student, sod_teacher.detach()
        )

        cross_task_consis_loss = cross_task_consistency(strong_preds_student, sod_student)

        tot_self_loss = (strong_self_sup_loss + weak_self_sup_loss + 0.05 * strong_sod_consis_loss + 0.01 * cross_task_consis_loss) * weight
    ...
    def validation_step(self, batch, batch_indx):
        ...
        strong_preds_student, weak_preds_student, _ = self.detect(mels, self.sed_student, temp=1)
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher, _ = self.detect(mels, self.sed_teacher, temp=1)
        ...
        if torch.any(mask_synth):
            ...
            for th in self.val_buffer_student_synth.keys():
                self.val_buffer_student_synth[th] = pd.concat([self.val_buffer_student_synth[th],
            ...
            for th in self.val_buffer_teacher_synth.keys():
                ...
                self.val_buffer_teacher_synth[th] = pd.concat([self.val_buffer_teacher_synth[th],
    ...
    def test_step(self, batch, batch_indx):
        ...
        # prediction for student
        mels = self.mel_spec(audio)
        strong_preds_student, weak_preds_student, vad_student = self.detect(mels, self.sed_student, temp=1)
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher, vad_teacher = self.detect(mels, self.sed_teacher, temp=1)

        psds1_post = 1
        if psds1_post == 1:
            strong_preds_student = cross_task_post_processing(strong_preds_student, vad_student)
            strong_preds_teacher = cross_task_post_processing(strong_preds_teacher, vad_teacher)
        ...
        # compute f1 score
        self.decoded_student_05_buffer = pd.concat([self.decoded_student_05_buffer,
                                                       decoded_student_strong[0.5]])

        self.decoded_teacher_05_buffer = pd.concat([self.decoded_teacher_05_buffer,
