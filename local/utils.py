...
from local.classes_dict import classes_labels, sod_labels


encoder_sod = ManyHotEncoder(
    list(sod_labels.keys()),
    audio_len=10,
    frame_len=2048,
    frame_hop=256,
    net_pooling=4,
...
def batched_decode_preds(
	...
	median_filter = [3, 28, 7, 4, 7, 22, 48, 19, 10, 50]
	scores_raw = {}
	...
	for j in range(strong_preds.shape[0]):  # over batches
		...
		pred_strong_m0 = scipy.ndimage.filters.median_filter(c_scores[:, 0], (median_filter[0]))
		pred_strong_m1 = scipy.ndimage.filters.median_filter(c_scores[:, 1], (median_filter[1]))
		pred_strong_m2 = scipy.ndimage.filters.median_filter(c_scores[:, 2], (median_filter[2]))
		pred_strong_m3 = scipy.ndimage.filters.median_filter(c_scores[:, 3], (median_filter[3]))
		pred_strong_m4 = scipy.ndimage.filters.median_filter(c_scores[:, 4], (median_filter[4]))
		pred_strong_m5 = scipy.ndimage.filters.median_filter(c_scores[:, 5], (median_filter[5]))
		pred_strong_m6 = scipy.ndimage.filters.median_filter(c_scores[:, 6], (median_filter[6]))
		pred_strong_m7 = scipy.ndimage.filters.median_filter(c_scores[:, 7], (median_filter[7]))
		pred_strong_m8 = scipy.ndimage.filters.median_filter(c_scores[:, 8], (median_filter[8]))
		pred_strong_m9 = scipy.ndimage.filters.median_filter(c_scores[:, 9], (median_filter[9]))

		pred_strong_m = np.vstack((pred_strong_m0, pred_strong_m1, pred_strong_m2, pred_strong_m3,
		                           pred_strong_m4, pred_strong_m5, pred_strong_m6, pred_strong_m7,
		                           pred_strong_m8, pred_strong_m9))
		c_scores = pred_strong_m.transpose()
